...
const auth = {
    ...
    register: async function register(res, { api_key, email, password }) {
        try {
          // Kontrollera API-nyckeln
          const validApiKey = "6fd8e90cd1ad3e58e1e91e5de1290329"; // Din API-nyckel
          ...
          users.push(newUser); // Lägg till användaren i databasen
          ...
          console.error(error);
          return res.status(500).json({ error: "Ett fel inträffade. Försök igen senare." });
        }
      },
